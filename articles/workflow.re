...
必ずこうすべきだというものではなくRe:VIEWを使った同人誌ってこうやってつくったんだよという体験記です。
同人誌というジャンルは広く、多様な表現が可能です。
技術誌としての企画、構成、編集、入稿まで制作フローに興味があるとより楽しめます。
...
技術書を頒布する@<fn>{hanpu}のに適したイベントは技術書典@<fn>{me}です。
...
これはレビュー内容を取り込むときにWebサイトと見比べながら行うのが面倒であること、レビュワー同士で他にどういうコメントがあるかが把握できたほうが学びが多く指摘の重複も少ないことからです。

#@# prh:disable
コメントを書いた後、直接main branchにpushしない理由は筆者の修正とconflictが発生しやすいためです。
筆者は好きなときに好きなものを書き、気が向いたときにレビューの反映作業を行うのが精神衛生上よいでしょう。

文章そのものへの指摘ではない章の構造などより大きな視点で指摘する場合は、レビューのフェーズを問わずIssueのみを使って、やりとりを進めるケースがあります。

重要な点としてレビュー指摘をどう反映するかは著者の裁量に任せています。
もちろん、この後の行程では編集者が手を加えるのでヒエラルキーはレビュー＜執筆者＜編集者です。

これは執筆の段階では納得のいく指摘のみ取り込んで欲しいからです。レビューをすべて取り込むことがよいことではありません。とくに多くの観点をケアしようとするあまり冗長になるというのは、わたしたちが考える大きなデメリットです。
わかりやすい文章がすべてをカバーしていることは稀で、何を書かないかの選択も重要です。
編集も著者の主張が適切に伝わるかという観点で作業します。

==== 編集者が校正＆調整作業に入る（直pushの禁止。変更はpull requestを送る）

main branchの凍結後にmain branchを変更する権限をもつのは編集者だけです。TechBoosterでは@<kw>{mhidaka,ひつじ}の役割です。執筆者がどうしても修正したい場合、pull requestを送り、お伺いを立てます。
...
そこでTechBoosterではDockerとGitHub Actionsを活用してCIのための仕組みを制作しました。平たくいうと常に最新のPDF出力が得られます。
...
Re:VIEWはPDFだけでなくEPUBやHTMLなど各種フォーマットに対応しているので、執筆中の出力だってドラフトやHTML出力だけでいいんじゃない？と考えることもできますが、印刷が前提だと最終確認するだけでは心許なく感じます。締切ギリギリにあがった原稿を編集者だけでチェックして印刷するなんて無茶は誰だってしたくありません。というわけで印刷用であったり最終レイアウトだったりのリリース向けの成果物を執筆中でも確認できるようにします。少なくとも著者がチェックできていないケースはなくなります。


Docker上でRe:VIEWを動かすツールおよびGitHub Actionsとして実行するラッパーツールを用意して常に最新のPDF出力が得られるような運用をしています。これには多くのメリットがありますが中でも次のような点で執筆に貢献しています。

 * ビルド環境の準備が不要になる
 * Re:VIEWの文法エラーにすぐ気づける
 * レビュー対象物を特定できる

TechBoosterでは40人程度が執筆に関わるので全員に特定のプラットフォームを強制することはできません。戦争が起きます。CIとVisual Studio Codeと拡張機能であるvscode-language-reviewがあればローカルで文法チェックを行ってCIがビルドする形で最低限の執筆環境ができます。
