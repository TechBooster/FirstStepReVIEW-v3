...
かといって、すべてを説明すると流れを理解するまえに頭がパンクしてしまいます。ここでは日光企画の事例に限って入稿に必要な知識をみていきましょう。実際には入稿予定の印刷所が指定するフォーマット、注意点を熟読して入稿に挑みましょう。
...
TechBoosterが過去に遭遇した事例では次のとおりです。
ちなみにTechBoosterでは日光企画さんを利用しています（ですので印刷所選びに不安があれば本書のリポジトリ＋日光企画さんを使うとトラブルなく入稿できる可能性が高くなります）。
