...
//footnote[adobe][表紙データの作成にもPhotoshopを使うので潔くCreative Cloudのコンプリート月々プラン7,980円での契約がオススメです]

ページ数は基本的に４の倍数になるように揃えて入稿します。ならない場合は無理やり白いページを挟むこともあります。またページごとにノンブル（ページの通し番号）が必要です。同人誌以外では見ない制約ですが、乱丁を防ぐためにつけています。レイアウトは指がかかるのと本のノド（綴じている側の余白）があるので余裕を持った設定をしています。読みやすさに直結するので注意して設定してください。最初のうちはデフォルトでの利用を推奨します。

Re:VIEWでは次のコマンドでPDFを出力できます
...
埋め込み方は「教授でもできる、TeXShop + MacTeX /UpTeX でのヒラギノ フォントの埋め込み」サイト@<fn>{mactex_font}が参考になります。次のコマンドはサイトからの引用です。
...
//footenote[mactex_font][http://osksn2.hep.sci.osaka-u.ac.jp/~taku/osx/embed_hiragino.html]
...
しかし、印刷所への入稿を考えるとPDF/Xフォーマットが最もポピュラーです。残念ながらLaTeX単体では対応できないため、ここから先はAcrobat Pro DCでの操作です。

ツールから印刷工程を選ぶと次のサイドメニューが表示されます（@<img>{pdfx}）。

//image[pdfx][PDF/Xとして保存する]{
//}
...
設定で色を置換するとモノクロ化できます。ここまでで本文の入稿データ作成は完了です。
