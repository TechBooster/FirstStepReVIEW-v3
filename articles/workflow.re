...
そしてこれを読んで作ってみたいな、と思った方はガイドラインとしてご活用ください。
...
これは同一の.reファイルを複数人で編集する機会が少ないからです。
...
@<chapref>{writing-book}でも少し触れましたが初校レビューはインラインコメントとして.reファイル中に書くケースが大半です。
...
筆者は好きなときに好きなものを書き、気が向いたときにレビューの反映作業を行うのが精神衛生上良いでしょう。
...
これは納得のいく指摘のみ取り込んで欲しいからです。レビューをすべて取り込むことがよいことではありません。わかりやすい文章がすべてをカバーしていることは稀で、何を書かないかの選択も重要です。
...
これは印刷所への入稿に向け、ページ数の調整を行っている段階だからです。
これを守らない場合、PDF化したときに改行の具合やら1行あたりの文字数やらの細かな調整がご破算になり編集者がぷっぷくぷー！になります。
可哀想なのでやめてあげましょう。
...
//footnote[Griflet][Griflet（グリフレット）はグリフ＋リーフレットの造語です。本に由来して名前を考えています。かっこいいとおもいませんか？僕は思います]
...
いまのところ負荷問題があり、それに至っていません。手軽に得られるビルド環境としてDockerイメージ@<fn>{docker-image}を提供しています。
最近だとDockerイメージを流用しやすいCircle CI 2.0@<fn>{circleci-2.0}やWercker@<fn>{wercker}などのCIサービスもでてきています。

//footnote[docker-image][@<href>{https://hub.docker.com/r/vvakame/review/}]
//footnote[circleci-2.0][@<href>{https://circleci.com/docs/2.0/}]
...
そこでprhを作成し羊の作業から著者個人の作業に落とした@<fn>{crazy-mhidaka}わけです。
...
ファイルを@<code>{prh.yml}という名前で.reファイルと同じディレクトリに置くとlanguage-reviewが校正用ルールとして参照して、Lintを表示します（@<img>{lint}）。
...
類似のツールには
RedPen@<fn>{redpen}やtextlint@<fn>{textlint}などがあります。
...
//footnote[crazy-mhidaka][「すごい！校正の労力が半分になったぞ！だから前回の2倍の冊数作った！」ってヤツは言いました。あたまおかしい]
