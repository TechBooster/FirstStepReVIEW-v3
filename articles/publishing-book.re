...
日光企画とねこのしっぽは、技術書典のバックアップ印刷所です。入稿のサポート、専用締切の提供をはじめオフライン会場当日の搬入を印刷所が行います。
...
今回は日光企画さんの事例に限って入稿に必要な知識をみていきましょう。実際には入稿予定の印刷所が指定するフォーマット、注意点を熟読して挑みましょう。

とはいえ困ったことばかりではありません。
繰り返すようにTechBoosterのRe:VIEWテンプレートを使っている限り、大きな問題には合いませんし、印刷所のスタッフさんも表紙や本文を目で確認してくれるため
...
TechBoosterが過去に遭遇した印刷所のルールは次のとおりです。驚かすつもりはないので印刷所選びに不安があれば日光企画やねこのしっぽを使うとトラブルなく入稿できる可能性が高くなります。
...
背幅は紙の厚さとページ数で決まります。日光企画さんの標準的なセットでは0.063mm/ページで計算できます。背幅は表紙も含めたページ数なので本文128ページであれば表紙をいれると132ページなので背幅は8.316mmだとわかります。ちなみに表紙は表紙１〜表紙４と呼び、ページ数に含めます。本の内側に来る表紙２、表紙３は通常、印刷しません。
...
ほとんどのケースでは、GitHubリポジトリでRe:VIEW-build-artifact-actionを使って生成したPDFファイルが使えます。
最近はGitHub Actionsで作ったPDFファイルをそのまま印刷所に渡して入稿完了というお手軽な事例も沢山出ているので、本文の制作難易度は下がってきています。
...
ローカルPC上にRe:VIEWの実行環境がある場合は次のコマンドで入稿用のPDFを出力できます。PDFファイルの出力にRe:VIEW-build-artifact-actionを使っている場合は、push時にPDFを自動生成するため、コマンドは読み飛ばして構いません。
...
最適化した場合にはトンボといった印刷に関わる設定以外も大きく変わっています。そのため改行位置やページ数など本文に関わる見栄えが大きく変わります。
...
Re:VIEW-build-artifact-actionは現時点でEPUB出力に対応していません。
...
これはPDFとEPUBのフォーマットの違いをよく表しています。EPUBはレイアウトやフォントサイズなどの表示品質はリーダーに委ねているのです（委ねたほうがデバイスや読者の要望ごとに最適な読書環境が提供できるとの考えから）。
...
MacTex 2017ではデフォルトでIPAフォントを利用しています。埋め込まれていない場合は@<code>{noEmbed}と表示しているはずです。
