...
#@# REVIEW vv: そういえば、入稿という用語を解説したことがなかった気がする 他の語句は常識の範囲かなぁと思わなくもない…？ 編集 校正 などは…？

印刷所へ最終的な印刷用データを渡すことを入稿と呼びます。
本節では入稿について実際の事例をベースに解説します。

#@# REVIEW vv: と思ったらここにあった。ここか？
...
たとえば表紙データは印刷所ごとにテンプレートが異なったり、入稿用本文にトンボ（断裁位置の目印）がないと受け付けてもらえなかったり、利用ソフトウェアのバージョン指定があったり、さまざまです。
#@# vv: 裁断じゃね？と思ったら断裁でいいんだなー初めて知った(Wikipedia調べ)
...
このコマンドを実行したあとに@<code>{review-pdfmaker}コマンドでPDFを作成するとフォントが埋め込まれています。文書のプロパティで埋め込みフォントと表示があれば成功です（@<img>{font}）。
...
電子書籍用レイアウトの適用は少々手がかかります。手順を追って説明しましょう。
 ...
 1. @<code>{config.yml}の変更
 2. レイアウトファイルの差し替え
 3. @<code>{review-pdfmaker}でPDF出力する

まず@<code>{config.yml}を紙面レイアウトから電子書籍向けレイアウトに変更します（@<list>{config_ebook}）。
...
成功したら@<code>{bookname.pdf}ファイルが得られます。念のため、フォントが埋め込まれているか確認しておいてください。
...
EPUBでも表紙をつけることができます。@<code>{config.yml}の設定を抜粋します（@<list>{epub_config}）。
...
@<code>{cover.jpg}は@<code>{config.yml}の位置をルートに@<code>{image/cover.jpg}へ配置してください。
カバー画像がない場合は書名が表示されます。
...
成功したら@<code>{bookname.epub}ファイルが出力されます。早速、出力されたEPUBをiBooksで表示してみましょう（@<img>{epub}）。
