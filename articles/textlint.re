...
著者にとって技術書に登場する文章は、なんども咀嚼して推敲を重ねた馴染み深い存在です。しかし本が読者に渡ったあとには、その文章が突如として初めて読む存在に変貌します。

書籍のなかに、おおむね3行を超える文章があったら2つ以上の文章に分割してください。おすすめは主語と述語（動詞）の距離を短く保つことです。
...
Visual Studio Codeのようなエディタのプラグインlanguage-reviewにはprhが組み込まれているため、language-review経由で使うのが楽でしょう。

まずは、TechBoosterの設定ファイル@<fn>{prh_setting}をダウンロードして試してみるのがよいでしょう。
ファイルを@<tt>{prh.yml}という名前で@<tt>{.re}ファイルと同じディレクトリに置くとlanguage-reviewが校正用ルールとして参照して、Lintを表示します（@<img>{lint}）。
...
本章で示した指標はあくまで補助線だと考えて自分が表現したい技術を書くことを追求してください。書き終わりさえすれば校正フェーズの時間は睡眠を削って絞り出したりできます。あと一歩で良いものができるぞと感じたら自然と効率が上がるものです。しかし書き終わらないと始まりません。編集のつらいところですね@<fn>{early}。
