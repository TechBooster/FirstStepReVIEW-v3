...
プリプロセッサ命令で読み込むファイルのタブ文字を自動で任意の数のスペースに置換します。

紙面の都合上、TechBoosterではサンプルコードのインデントは2スペースとしています。
しかし、サンプルコードを最初から2スペースのインデントで書くというのはあまりやりたくありません。
タブ文字でインデントしておけば原稿に反映する際に自動でインデントの文字幅を置換できます。
サンプルコードを編集するエディタ上ではタブ文字をお好みの幅で表示しましょう。

@<code>{tabwidth}オプションは@<code>{--tabwidth=WIDTH}という形式で指定します。
たとえば、sample.reに対して@<code>{review-preproc}コマンドを実行しタブ文字を2スペースに置換するには、次のようにします。
