...
本章ではエンジニアや技術書典など技術イベント出展者向けのRe:VIEWを使った執筆・編集スタイルを紹介します。
...
//image[review-cover][Re:VIEWがカバーする領域][scale=0.75]{
//}
...
#@# NOTE author:vvakame, mod:mhidaka
...
技術書であれば主なターゲットはPDFですが、あとからEPUBにできればKindleのmobiファイルなどの電子リーダー独自のフォーマットにも対応できます。拡張性を求めるなら最初から考慮されているツールセットをお勧めします。
...
構造化された文書は見出しのスタイルを一括で変更でき、目次の生成を簡単にします。
...
そのままGitHubなどを通じて作業するだけでなくGitHub ActionsやCircle CIを利用できますし、TechBoosterで制作したRe:VIEW特化のGitHub Actions@<fn>{github-actions}があります。CommitごとにPDFを確認できる利便性は計り知れません。
...
Re:VIEWはHTMLやPDF、EPUBやMarkdownといった主要な形式への変換にも対応しています。
さらに文書の構造と見た目が十分に分離されているので、原稿ができてから見た目を調整できます。

そして出版を生業としている方々が作っているので「本を書くための工夫」がたくさん詰まっています。
日本人が作っているツールだけあって、日本語固有の事情も考慮されています。そして今まで数多くの本の制作に使われてきた実績@<fn>{archievement}があり、商業誌と同じクオリティを確保しています。そうそう落とし穴に落ちたりすることもありません。
...
印刷所である日光企画さん・ねこのしっぽさんに入稿できる、多数の利用実績があるテンプレートリポジトリです。はじめての執筆で利用するにはうってつけです。

しかしながらすべての印刷所で大丈夫とは限りません。印刷所ごとに入稿ルールは異なります。
入稿に先立って印刷所のスタッフが確認できるサンプルファイルを用意し、印刷所の指示にしたがって入稿データを作成してください。出力方法は後述の章で解説します。
