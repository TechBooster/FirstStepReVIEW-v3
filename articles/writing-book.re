...
本章では、TechBoosterがお勧めするエンジニア向けの執筆スタイルを提案します。
効率的な様子は、まるでソースコードを書いているようだと勘違いするかも！
...
原稿についてもpull requestを使って改善の提案やレビューを受け取ることは編集の競合を防ぎ、
コントリビュートしやすさにつながります。
また、Travis CIやCircle CI、その他Re:VIEW専用CIなどを利用し、pushするたびにPDFを生成できるようになります。

 * コメントを利用できる

コメントへはメモを書き残したり、他人の原稿にレビューを書き込む時に使います。
たとえばIssueに@<code>{原稿の○行目に対する指摘 > わかる→分かる に置き換え}という風に個々の指摘を書くのは大変不便です。
...
そのため、任意の形式に変換できるとよいでしょう。

 * 文書の構造自体と見た目（スタイル）についての設定が分離されている

GUIツールにありがちですが、そのツールを使う人の練度に文書構造自体の質が左右される場合があります。
たとえば、章タイトルの文字を大きくしたい時、文字を大きく太く装飾することと、そこが見出しという文書構造の1部分であるとスタイルを設定するのは、
まったく違うことです。しかし、見た目としては変わりがない場合、単に文字を大きく装飾して終わってしまう人がいます。
そのような文章は目次の自動生成や、複数形式に出力した時の出来栄えに難があります。
使わないでね！と伝えて済む場合もありますが、それよりも根本的に構造とスタイルが分離されているツールを使うほうが楽です。
